...
class NodeClass
{
	...
	public:
		...
		NodeClass()
		{
			...
			nh.param("port", port, std::string("/dev/ttyUSB_LASER0"));
			...
			nh.param("frame_id", frame_id, std::string("back_laser_link"));
			...
			topicPub_LaserScan = nh.advertise<sensor_msgs::LaserScan>("back_scan", 1);
