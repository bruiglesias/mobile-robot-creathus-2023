...
class NodeClass
{
	...
	public:
		...
		NodeClass()
		{
			...
			nh.param("port", port, std::string("/dev/ttyUSB_LASER1"));
			...
			nh.param("frame_id", frame_id, std::string("front_laser_link"));
			...
			topicPub_LaserScan = nh.advertise<sensor_msgs::LaserScan>("front_scan", 1);
