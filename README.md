...
```
mkdir -p ~/runner_ws/src && cd ~/runner_ws
...
![runnerRobot](/images/guntherBOT_simulation_complete.png)
...
```
vim ~/runner_ws/src/runner/robot_description/launch/teleop-guntherbot.launch

subl ~/runner_ws/src/runner/robot_description/launch/teleop-guntherbot.launch
```
...
## Real RunnerBOT - Integration Raspberry Pi 3, Arduino and ROS
...
If you want to see the behavior of the low level pid control, use the tool `rqt_multiplot` and upload the file `~/runner_ws/src/runner/rqt_multiplot.xml`.
...
```
rosrun rviz rviz -d ~/runner_ws/src/runner/arduino_controller/rviz/rviz_test_arduino.rviz
```
