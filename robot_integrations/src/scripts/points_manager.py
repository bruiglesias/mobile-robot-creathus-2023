...
from robot_integrations.srv import Point, PointRequest, PointResponse
...
def read_json():
    try:
        with open("/home/ros/ros_ws/src/robot_integrations/src/db_points/points.json", "r") as file:
            data = json.load(file)
            return data
    except:
        return []
...
def save_json(points_dict):
    with open("/home/ros/ros_ws/src/robot_integrations/src/db_points/points.json", "w") as outfile:
...
if __name__ == "__main__":
    ...
    current_position = rospy.Service('/points_manager/robot_current_position', Empty , current_position_callback)

    all_points = rospy.Service('/points_manager/get_all_points', Empty , all_points_callback)
    add_new_point = rospy.Service('/points_manager/add_new_point', Point , add_new_point_callback)
    delete_point = rospy.Service('/points_manager/delete_point', Point , delete_point_callback)

    
    sub_pose = rospy.Subscriber('/odom' , Odometry, subcriber_callback) 
